class Question < ApplicationRecord
  ...
  scope :user_unseen_questions, ->(user) { includes(:options).where.not(id: user.answers.pluck(:question_id)) }
  scope :user_attended_questions, ->(user) { includes(:options, :answers).where(answers: { user_id: user }) }
